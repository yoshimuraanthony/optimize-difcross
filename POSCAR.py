...
def getFromXDAT(
        ...
        OUTCAR = 'OUTCAR'):
    """
    retuns POSCAR instance corresponding to particular frame in XDATCAR
    infile: XDATCAR file (str)
    frame: frame or configuration number from XDATCAR, starting from 1
        (pos int)
    ...
    if timeStep > 1E-6:
        ...
        dirIvs  = ((array(coordsNext) - array(coordsPrev)) / timeStep / 2).tolist()
...
def genTranslate(table, translation, atom_list):
    ...
    translation: translation vector in fractional coordinates (list of 3 floats)
    """
    atom_list = [atom - 1 for atom in atom_list] # python starts from 0
    ...
    for atom in atom_list:
        for vector in range(3):
            new_table[atom][vector] = new_table[atom][vector] + translation[vector]
...
class POSCAR(object):
    ...
    def write(self,
        ...
        """
        # UNDER CONTRUCTION: option to align columns using makeReadable function
        # write POSCAR
        if '/' in outfile:
    ...
    def alignCell(self, zero = True, lat_num = 1, direction = [1, 0, 0]):
        """
        aligns cell such that a1 has no y, z component,
            and a2 has no z component
        zero: if True, sets 2nd and 3rd compoenents of a1
            and third components of a2 to exactly 0
...
#        transCoords = transpose(self.coords)
    ...
    def getBondAngle(self, end1, mid, end2, deg = True):
        """
        returns the bond angle (in degrees) between the two end points and a vertex
    ...
    def getCartOf(self, site):
        ...
        site: site as indexed in VESTA or coordinate vector (pos int or list of 3 floats)
        """
        if type(site) == int:
            if self.rep[0] == 'c' or self.rep[0] == 'C':
    ...
    def getDist(self, atom1, atom2):
        ...
        atoms: atoms indexed as in VESTA (int)
        """
        if self.rep[0] == 'c' or self.rep[0] == 'C':
    ...
    def getK_list(self, number = 10, layer = False):
        """
        returns list of k-point mesh dimensions suitable for cell
        number: length of list (pos int)
    ...
    def makeBigPrimCell(self, num_pts, layer = True, catchAngle = False):
        """
        resizes and rotates primitive cell to fit spedified number of
            lattice points
    ...
    def matchOrderTo(self, other, threshold = 1):
        ...
        # empty POSCAR in which final positions will be added in the correct order
        new = self.copy()
        selfAtoms = [n + 1 for n in range(self.getNumAtoms())] # list of coord indices
    ...
    def prepAdsorptionLandscape(self, species, corner1, corner2, ncolumns = 7, minDist = 2.25):
        ...
        corner{1,2}: (direct) coordinates specifying corners of landscape (list of three of floats)
        ncolumns: number of intermediate positions between corners
        minDist: minumum interatomic distance (angstrom) between adsorbed atom and surface (float)
        """
        ...
        for n in range(ncolumns + 2):
            ...
            adCoord[2] += 2*minDist      # hypothetically raise adatom above surface
...
#                        destination + '/' + directory + '/KPOINTS')
    ...
    def prepKConvergence(self,
        ...
        layer: if True, and if k_list = 'auto', third mesh dimenstion = 1 (bool)
        number: length of k_list if automatically generated (pos int)
        destination: directory to which files are saved (str)
        """
        if self.ktable == None:
            self.addKPOINTS()
        ...
        if k_list == 'auto':
            k_list = self.getK_list(number, layer)

        for mesh in k_list:
            POS = self.copy()
            POS.kmesh = mesh
            mesh_str = ['{0:0>2}'.format(mesh[n]) for n in range(3)]
            directory = '%sx%sx%s' %(mesh_str[0], mesh_str[1], mesh_str[2])
#            POS.write(destination + '/' + directory + '/POSCAR_scr',
            POS.write('%s/%s/POSCAR_scr' %(destination, directory),
                        '%s/%s/KPOINTS' %(destination, directory))
            POS.write('%s/%s/POSCAR' %(destination, directory), writeKPOINTS=False)
    ...
    def prepLAMMPS(self,
        ...
        # align cell so that a1 had to y, z component and a2 has no z component
        self.alignCell()
    ...
    def prepLayerConvergence(self,
        ...
        """
        prepares POSCAR and KPOINTS files to test convergence of the number of layers
            in a surface calculation
        ...
        for num_layers in l_list:
            ...
            POS.makeSuperCell(super_cell, glide, screw)   # add layers in z-direction
            ...
            # enable sd on layers within a speficied depth
                ...
                for height in height_list:
                    ...
                    cart_height_list.append(POS.getCartOf(height_vector)[direction - 1])
    ...
    def prepNEB(self,
        ...
        fixedLengths: indicates which bond lengths are kept constant (list of tuple pairs)
            * atom1 position is linearly interpolated, atom2 moves correspondingly
        alongBondCenter: if True, bond center of fixed bond move linearly (bool)
        fixedAtoms: atoms (as in VESTA) that need to be fixed (list of pos int)
        align: if True, aligns initial and final states before interpolating (bool)
        alignement: 'all' or 'cm' (str)
        matchOrder: if True, matches order of other coordinates to self
        threshold: max dist in Å for which an atom is recognized as the same atom (float)
        """
            ...
            # shifts final POSCAR by average displacement
                ...
                for coord1, coord2 in zip(coords1, coords2):
                    for comp in range(3):
                        ...
                        dif -= round(dif) # distance between nearest periodic image
                        dif_ar[comp] += dif
...
#        dif_ar = coords2_ar - coords1_ar
        ...
        for n in range(number):
            ...
            for atom1, atom2 in fixedLengths:
                imageAxis = self.getCartOf(array(shift_ar[atom2 - 1]) - array(shift_ar[atom1 - 1]))
...
#                print(image.getDist(atom1, atom2))
    ...
    def prepVacConvergence(self, width_list = 'auto', origin = [.5, .5, .5], destination = '.', asym = True):
        """
        ...
        for n in width_list:
            ...
            if asym:
                POS.resizeVac([n, n + 1, n + 2], origin = origin) # break symmetry for molecules
    ...
    def prepZConvergence(self, z_list = 'auto', destination = '.', operation = 'set equal', depth = None, direction = 3, origin = [.5,.5,.5]):
        """
        prepares POSCAR files to test convergence of vacuum distance between layers
        z_list: list of vacuum distances to test (list of floats)
        destination: directory to which files are saved (str)
        operation: operation by which z_list transforms cell height ('multiply or add')
        depth: depth to which atoms will be allowed to relax
            if type(depth) == int, sd is True for depth # of layers
            if type(depth) == float, sd is True within depth angstroms from surface or edge
...
#            pass
        ...
        ratio = norm(self.cell[direction - 1]) / self.cell[direction - 1][direction - 1]
    ...
    def reflect(self, plane, origin = [.5, .5, .5], atom_list = 'all'):
        ...
        plane: miller indices for reflection plane (list of 3 ints)
        origin: a site or list of direct coordinates that lies on the reflection plane
            (int of list of 3 floats)
        atom_list: atoms, indexed as in VESTA, to be reflected (list of ints)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
        ...
        origin = dot(origin, self.cell).tolist() # origin in cartesian coordinates
        ...
        self.coords = genReflect(self.coords, rec_cell, plane, origin, atom_list)
    ...
    def relocate(self, atom_list, dest_list):
        ...
        dest_list: list of atom desinations in fractional coordinates (list of lists)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
    ...
    def remove(self, atom_list):
        ...
        # remove atoms from self.coords in descending order to maintain atom numbering
    ...
    def removeDoubles(self, minDist = .01):
        ...
        minDist: interatomic distance in Å below which atoms are considered too close (float)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()

        range_dict = self.getSpecRanges()
        coords = [self.getCartOf(coord) for coord in self.coords] # cartesian coordinates
            ...
            for j in range(start, len(coords)):
                if i != j:
                    ...
                    if dist < minDist or dist2 < minDist:
                        ...
                        spec_i, spec_j = self.getSpecOf(i + 1), self.getSpecOf(j + 1)
                        if spec_i == spec_j:
                            rm_list.append(i + 1)
#                        else:
#                            print(
...
#        %(spec_i, i + 1, spec_j, j + 1))
        ...
        for newIndex, spec in enumerate(newSpec_list):
            ...
            # add new coordinates in order of newSpecies_list
                ...
                for n in range(firstIndex, lastIndex):
                    newCoords_list.append(self.coords[n][:]) # avoid aliasing with [:]
    ...
    def resizeVac(self, new_len_list, operation = 'set equal', origin = [0, 0, 0]):
        """
	...
	"""
        ...
        if len(self.coords) > 1:                        # avoid empty cell error
            coords = array(self.coords) - array(origin) # center coords on origin
        else:
            coords = []
    ...
    def rotate(self,
        ...
        cartesian: if True, axis is assumed to be in cartesian coordinates (bool)
        origin: a site (VESTA) or list of direct coordinates that lies on the axis
            of rotation (int or list of 3 floats)
        atom_list: atoms, indexed as in VESTA, to be rotated (list of ints)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
    ...
    def roundCoords(self, degree = 5, atom_list = 'all', components = 'all'):
        ...
        degree: degree of rounding
            if degree = 'auto', rounds all coords of 5 or more repeating decimals
        atom_list: atoms (starting from 1) whose coordinates will be rounded (list of pos ints)
        components: components (starting from 1) that will be rounded (list of pos ints)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
                ...
                if 'e' in comp_str:
                    ...
                    if consecutive == degree:
                        ...
                        # round up
                            ...
                            new_comp_str = comp_str[:n] + rep * (17 - n) + str(int(rep) + 1)
                            print('round up: %s to %s' %(comp_str, new_comp_str))
                            self.coords[i][j] = float(new_comp_str)

                        # round down
                        else:
                            new_comp_str = comp_str[:n] + comp_str[n] * (18 - n)
                            print('round down: %s to %s' %(comp_str, new_comp_str))
                            self.coords[i][j] = float(new_comp_str)
    ...
    def sdMakeFalse(self, atom_list = 'all', directions = 'all'):
        ...
        directions: directions to disable (iterable containing any combination of 1,2,3)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
    ...
    def sdMakeTrue(self, atom_list = 'all', directions = 'all'):
        ...
        atom_list: list of atoms, al a VESTA (iterable of ints or 'all')
        directions: directions to enable (iterable containing any combination of 1,2,3)
        """
        if stack()[1][3] == '<module>':
            self.updateTables()
    ...
    def selectDepth(self, depth = 5.0, direction = True):
        """
        returns list of atoms that are within a specified depth from a surface or edge
        depth: depth from surface or edge
            if type == int, depth is number of layers
            if type == float, depth is distance in angstroms
        direction: miller indices of direction with respect depth is measured (tup of 3 floats)
        """
    ...
    def selectRec(self, origin, vertex):
        """
        returns list of atoms (indexed a la VESTA) within desired parallelepiped
    ...
    def selectSpec(self, spec_list):
        ...
        for spec in spec_list:
            site_list = [n + 1 for n in range(range_dict[spec][0], range_dict[spec][1]) ]
            atom_list += site_list
...
#        or type(v_tab[0]) == numpy.float64:
    ...
    def sortCoords(self, column = 3):
        ...
        for spec in range_dict:
            lower, upper = range_dict[spec]
            spec_list = [(self.coords[n][column - 1], n) for n in range(lower, upper)]
            spec_list.sort()
    ...
    def subtractFromPops(self, atom_list):
        ...
        atom_list: atoms, as indexed in VESTA, to be removed from pops (list of ints)
        """
        range_dict = self.getSpecRanges()
        ...
        for n in range(len(self.pops) - 1, -1, -1): # count backwards to maintain order
            if self.pops[n] < 1:
    ...
    def swap(self, site1, site2):
        ...
        self.coords[site1 - 1], self.coords[site2 - 1] = self.coords[site2 - 1], self.coords[site1 - 1]
    ...
    def translateTo(self, atom, newCoord, atom_list = 'all', cartesian = False):
        """
